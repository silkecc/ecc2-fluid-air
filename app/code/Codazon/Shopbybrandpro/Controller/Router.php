...
class Router implements \Magento\Framework\App\RouterInterface
{
    ...
    public function match(\Magento\Framework\App\RequestInterface $request)
    {
        ...
        if (isset($urlKey[1]) && ($urlKey[0] == 'brand')) {
            ...
            $optionValueTable = $brandCollection->getTable('eav_attribute_option');
            $select = $brandCollection->getConnection()->select();
            $select->from($optionValueTable, ['option_id'])
	    ->joinLeft([ 'cea' =>  $brandCollection->getTable('eav_attribute') ],$optionValueTable.'.attribute_id = cea.attribute_id')
	    ->joinLeft([ 'eaov' => $brandCollection->getTable('eav_attribute_option_value') ], $optionValueTable.'.option_id = eaov.option_id');
            $select->where('eaov.store_id IN ('.$defaultStoreId.', '.$storeId.')')
		->where("cea.attribute_code = '{$this->_attributeCode}'")
                ->where("LOWER(REPLACE(RTRIM(eaov.value), ' ', '-')) = '{$urlKey}'")
                ->order('store_id DESC')
                ->limit(1);
