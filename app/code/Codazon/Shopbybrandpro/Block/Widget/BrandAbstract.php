...
class BrandAbstract extends Template implements \Magento\Widget\Block\BlockInterface
{
    ...
    {
	    $writer = new \Zend\Log\Writer\Stream(BP . '/var/log/brands_timing.log');
		...
		$brandProductCounts = $this->_helper->getProductCountByAttributeCode($this->_attributeCode);
                foreach ($rows as $row) {
                    $optionId = $row['option_id'];
		    //$row['product_count'] = $this->_helper->getProductCount($this->_attributeCode, $optionId);
		    $row['product_count'] = isset($brandProductCounts[$optionId]) ? $brandProductCounts[$optionId] : 0;
