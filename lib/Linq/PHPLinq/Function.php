...
class PHPLinq_Function {
	...
	public function getFunctionReference() {
		if (is_null($this->_functionReference)) {
			...
			$this->_functionReference = $this->create_ref_function($this->_parameterNames, $this->_functionCode);
		}
		
		return $this->_functionReference;
	}

    public function create_ref_function( $args, $code )
    {
        static $n = 0;
        $functionName = sprintf('lambda_%d',++$n);
        $string = 'function ' . $functionName . "({$args}) { $code }";
        eval($string);
        return $functionName;
    }
